...
import type { CreateUserDto } from '@/apis/user'
...
const initializeForm: CreateUserDto = {
  username: '',
  password: '',
  role: '',
  nickname: '',
...
// 表单数据
const form = ref<CreateUserDto>({ ...initializeForm })

const { createUserMutation, updateUserMutation } = useReqUser({
  handleSuccess() {
    close()
    handleClose()
  }
...
const isLoading = computed(
  () => updateUserMutation.isPending.value || createUserMutation.isPending.value
)

const mutate = (form: CreateUserDto) =>
  form.id ? updateUserMutation.mutate(form) : createUserMutation.mutate(form)
